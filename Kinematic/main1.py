...
vars.f1 = open('./files/'+vars.nombre+'juego1.txt','a')
vars.f2 = open('./files/'+vars.nombre+'juego2.txt','a')
vars.f3 = open('./files/'+vars.nombre+'juego3.txt','a')
...
while vars.runGame :
	...
	if(not connected):
		...
		try:
			...
			# ~ print(data_arr)
			vars.y0 = float(data_arr[3])
			vars.z0= float(data_arr[2])
			vars.x0 = float(data_arr[1]) % 360
			vars.x2[0] = vars.x2[1]
			vars.x2[1] = vars.x0
